  ...
  '/api/products/2': {
    comments: [
      { 
        ...
        date: '2024-01-15',
        helpful: 23,
        avatar: 'https://via.placeholder.com/32x32/4CAF50/white?text=张'
      },
      { 
        id: 2,
        user: '李四', 
        ...
        date: '2024-01-14',
        helpful: 15,
        avatar: 'https://via.placeholder.com/32x32/2196F3/white?text=李'
      },
      { 
        id: 3,
        user: '王五', 
        ...
        date: '2024-01-13',
        helpful: 31,
        avatar: 'https://via.placeholder.com/32x32/FF9800/white?text=王'
      },
      {
        id: 4,
        user: '赵六',
        ...
        date: '2024-01-12',
        helpful: 8,
        avatar: 'https://via.placeholder.com/32x32/9C27B0/white?text=赵'
      }
    ],
    rating: 4.8,
    totalComments: 1250,
  ...
  '/api/cart': {
    items: [
      ...
      },
      {
        ...
        originalPrice: 1899,
        discount: 0,
        image: 'https://via.placeholder.com/100x100/FF6B6B/white?text=AirPods',
        color: '白色',
        features: ['主动降噪', '空间音频']
      }
    ],
    ...
    featured: [
      {
        ...
        image: 'https://via.placeholder.com/300x200/007ACC/white?text=iPhone+15',
        link: '/products/1'
      },
      {
        id: 2,
        ...
        image: 'https://via.placeholder.com/300x200/FF6B6B/white?text=Sale',
        link: '/categories'
      }
    ]
  },
        ...
        items: [
          {
            ...
            quantity: 1,
            price: 7999,
            image: 'https://via.placeholder.com/60x60/007ACC/white?text=iPhone'
          }
        ]
      },
      {
        ...
        items: [
          {
            ...
            quantity: 1,
            price: 1899,
            image: 'https://via.placeholder.com/60x60/FF6B6B/white?text=AirPods'
          }
        ]
      }
    ]
    ...
    products: [
      {
        ...
        name: 'MacBook Pro 14',
        price: 14999,
        image: 'https://via.placeholder.com/200x200/9C27B0/white?text=MacBook',
        rating: 4.9,
        reason: '基于您的浏览历史'
      },
      {
        ...
        name: 'AirPods Pro 2',
        price: 1899,
        image: 'https://via.placeholder.com/200x200/FF6B6B/white?text=AirPods',
        rating: 4.7,
        reason: '经常一起购买'
      }
    ]
...
Object.keys(apiData).forEach(endpoint => {
  app.get(endpoint, async (req, res) => {
    ...
    requestStats.byEndpoint[endpoint][isPrefetch ? 'prefetch' : 'normal']++;
    
    // 模拟不同的网络延迟
    const delay = isPrefetch ? 
      Math.random() * 100 + 50 :  // 预请求: 50-150ms
      Math.random() * 600 + 200;  // 正常请求: 200-800ms
    
    await addDelay(delay);
    
    const requestType = isPrefetch ? '🔄 [PREFETCH]' : '📡 [REQUEST]';
    const userAgent = req.headers['user-agent'] || 'Unknown';
    const clientInfo = userAgent.includes('Chrome') ? '🌐 Chrome' : 
                      userAgent.includes('Firefox') ? '🦊 Firefox' : 
                      userAgent.includes('Safari') ? '🧭 Safari' : '❓ Unknown';
    
    console.log(`${requestType} ${endpoint} - ${Math.round(delay)}ms ${clientInfo}`);
    
    // 设置预请求响应头
    if (isPrefetch && expireTime) {
      res.set('X-Prefetch-Response', 'true');
      res.set('X-Prefetch-Expire-Time', expireTime);
      res.set('Cache-Control', `max-age=${Math.floor(parseInt(expireTime) / 1000)}`);
    }
...
app.listen(port, () => {
  ...
  console.log('');
  console.log('💡 预请求支持:');
  console.log('   Header: X-Prefetch-Request-Type: prefetch');
  console.log('   Header: X-Prefetch-Expire-Time: 30000');
  console.log('');
  console.log('🔧 支持的客户端:');
...
process.on('SIGINT', () => {
  ...
  console.log(`   预请求: ${requestStats.prefetch}`);
  console.log(`   正常请求: ${requestStats.normal}`);
  console.log('👋 再见！');
  process.exit(0);
});
