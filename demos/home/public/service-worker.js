...
try {
     ...
     * api 白名单
     */
    const apiHostWhiteList = ['qa.nt.ctripcorp.com', 'trip.com', 'ctrip.com'];
    const isInWhiteList = (url) => {
        return apiHostWhiteList.some((host) => url.indexOf(host) > -1);
    };

    function getAppUrlByEvent(event) {
     ...
     * @param {*} data
     */
    function sendMessageToSender(event, data) {
